...
% ---------- Slide 10 ----------
\begin{frame}{Training Results \& Key Challenges}
\begin{enumerate}\tightitems
  \item \textbf{Memory Constraints:} Long-sequence decoding (512+ tokens) causes OOM. Hierarchical agent multiplies the problem.

  \item \textbf{Solution:} Per-rollout backward passes (B=1, G=2) and CodeBatcher for parallel CPU code execution.

  \item \textbf{RL Training:} Reward design and group size heavily impact convergence. Evaluation cost dominates.
\end{enumerate}

\vspace{0.3em}
\begin{center}
\includegraphics[width=0.85\textwidth,height=0.45\textheight,keepaspectratio]{../../docs/assets/training_loss_plot.png}
\end{center}
\end{frame}

% ---------- Slide 11 ----------
\begin{frame}{Future Work: Graph for Self-Improvement}
\begin{block}{From original proposal}
\footnotesize
\begin{quote}
\textit{A persistent key-value store of (\texttt{problem\_embedding} $\rightarrow$ \texttt{successful\_plan}) pairs. The architecture has one deliberate extension hook where the Router prompt can prepend retrieved plans from similar solved problems.}
\end{quote}
\end{block}
...
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.56\textwidth}
    \begin{block}{Proposed self-improvement graph}
    \scriptsize
    \begin{itemize}\tightitems
      \item Nodes are problems, plans, and rewards in one retrieval graph.
      \item Edges combine embedding similarity with success metadata.
      \item Router reads top-$k$ successful, similar plans before generation.
      \item Write-back triggers only on clean successes (correct + valid plan + no tool errors).
    \end{itemize}
    \end{block}
  \end{column}
  \begin{column}{0.40\textwidth}
    \begin{center}
    \includegraphics[width=\textwidth,height=0.30\textheight,keepaspectratio]{../../docs/assets/plan_graph_self_improvement.png}
    \end{center}
  \end{column}
\end{columns}
\end{frame}
