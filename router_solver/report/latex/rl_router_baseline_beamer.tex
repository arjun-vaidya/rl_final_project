...
\definecolor{navy}{RGB}{25,44,82}
\definecolor{bluegray}{RGB}{83,103,132}
\definecolor{softblue}{RGB}{232,238,247}
\definecolor{softgreen}{RGB}{232,244,237}
...
\newcommand{\tagbox}[2]{%
  \begin{tikzpicture}[baseline=(X.base)]
    \node[rounded corners=3pt, fill=#1, inner xsep=6pt, inner ysep=3pt] (X) {\small #2};
  \end{tikzpicture}%
}
...
% ---------- Metadata ----------
\title{Router Solver Baseline}
\subtitle{GSM8K Fine-Tuning, Reward Weighting, and Router Reward Design}
\author{RL Final Project}
\date{May 2026}
...
We identified and fixed an inversion bug in the loss calculation so that successful examples are reinforced rather than suppressed.
\end{alertblock}
\end{frame}
...
\vspace{0.4em}
\footnotesize Reference used in original deck: \url{https://yugeten.github.io/posts/2025/01/ppogrpo/}
\end{frame}
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{column}{0.48\textwidth}
    ...
    \begin{itemize}\tightitems
      ...
      \item Compare each response against the group baseline.
      \item Normalize rewards within the group, avoiding a critic.
    \end{itemize}
    \end{block}
  \end{column}
\end{columns}
...
\begin{center}
\Large
\tagbox{softgreen}{$A_i=\dfrac{R(r_i)-\mathrm{mean}(R)}{\mathrm{std}(R)}$}
\end{center}
...
\tagbox{softblue}{No value model} &
\tagbox{softgreen}{Lower overhead} &
...
\vspace{0.7em}
\begin{block}{Recommended Use}
Train with fast heuristic rewards. Validate with an LLM judge to detect reward inflation, reward deflation, and brittle heuristic behavior.
\end{block}
\end{frame}

% ---------- Slide 9 ----------
\begin{frame}{Recommended Implementation Path}
\begin{enumerate}\tightitems
  \item \textbf{Stabilize the flat baseline.} Confirm numeric extraction, reward calculation, and loss weighting.
  \item \textbf{Train a fast router reward.} Start with heuristic plan-quality plus outcome correctness.
  \item \textbf{Add modular evaluators.} Keep a common \texttt{RouterReward} interface so rewards can be swapped cleanly.
  \item \textbf{Validate with LLM-as-judge.} Use expert scoring for audits, not high-volume inner-loop training.
  \item \textbf{Compare reward regimes.} Track outcome accuracy, plan quality, reward calibration, and failure modes.
\end{enumerate}

\vspace{0.8em}
\begin{block}{Usage Sketch}
\texttt{from src.rewards.router import LLMJudgeRouterReward} \\
\texttt{evaluator = LLMJudgeRouterReward(model='claude-opus-4-1')}
\end{block}
\end{frame}

% ---------- Slide 10 ----------
\begin{frame}{References Mentioned in Original Deck}
\small
\begin{itemize}\tightitems
  \item Zhang et al. \textit{ReST-MCTS*: LLM Self-Training via Process Reward Guided Tree Search}. NeurIPS 2024. \url{https://arxiv.org/abs/2406.03816}
  \item \textit{HiPRAG: Hierarchical Process Rewards for Efficient Agentic RAG}. \url{https://arxiv.org/abs/2510.07794}
  \item \textit{Linking Process to Outcome: Conditional Reward Modeling for LLM Reasoning}. \url{https://arxiv.org/abs/2509.26578}
  \item PPO / GRPO explainer referenced in original deck: \url{https://yugeten.github.io/posts/2025/01/ppogrpo/}
\end{itemize}
\end{frame}
