...
def extract_final_answer(text: Optional[str]) -> str:
    ...
    if lines:
        first_line_numeric = _line_numeric_value(lines[0])
        if first_line_numeric is not None:
            return str(int(first_line_numeric)) if float(first_line_numeric).is_integer() else str(first_line_numeric)
