...
from src.utils.rollout_trace import append_rollout_trace, summarize_rollout_group
...
def collect_rollout_traces(agent, questions: List[str], ground_truths: List[str], num_questions: int, rollouts_per_q: int, trace_path: str) -> Dict:
    ...
    exact_correct = 0
    relaxed_correct = 0
    majority_exact_correct = 0
    majority_relaxed_correct = 0
    any_relaxed_correct = 0
    ...
    for q_idx, (question, gt) in enumerate(zip(questions[:num_questions], ground_truths[:num_questions])):
        ...
        group_summary = summarize_rollout_group(generated_rollouts, gt)
        ...
        majority_exact_correct += int(bool(group_summary.get("majority_exact_match", False)))
        majority_relaxed_correct += int(bool(group_summary.get("majority_relaxed_match", False)))
        any_relaxed_correct += int(bool(group_summary.get("any_relaxed_match", False)))
    ...
    return {
        ...
        "question_majority_exact_accuracy": majority_exact_correct / max(total_questions, 1),
        "question_majority_relaxed_accuracy": majority_relaxed_correct / max(total_questions, 1),
        "question_any_relaxed_accuracy": any_relaxed_correct / max(total_questions, 1),
    }
...
def run_trace_taxonomy(trace_path: str, max_failures: int = 50, max_examples_per_category: int = 3) -> Dict:
    ...
    question_majority_exact_correct = 0
    question_majority_relaxed_correct = 0
    question_any_relaxed_correct = 0
    ...
    for record in records:
        ...
        group_summary = record.get("group_summary", {})
        question_majority_exact_correct += int(bool(group_summary.get("majority_exact_match", False)))
        question_majority_relaxed_correct += int(bool(group_summary.get("majority_relaxed_match", False)))
        question_any_relaxed_correct += int(bool(group_summary.get("any_relaxed_match", False)))
    ...
    return {
        ...
        "exact_accuracy": exact_correct / max(total_rollouts, 1),
        "relaxed_numeric_accuracy": relaxed_correct / max(total_rollouts, 1),
        "question_majority_exact_accuracy": question_majority_exact_correct / max(len(records), 1),
        "question_majority_relaxed_accuracy": question_majority_relaxed_correct / max(len(records), 1),
        "question_any_relaxed_accuracy": question_any_relaxed_correct / max(len(records), 1),
...
def format_taxonomy_report(results: Dict) -> str:
    ...
    lines.append(f"- question_majority_exact_accuracy: `{results['question_majority_exact_accuracy']:.4f}`")
    lines.append(f"- question_majority_relaxed_accuracy: `{results['question_majority_relaxed_accuracy']:.4f}`")
    lines.append(f"- question_any_relaxed_accuracy: `{results['question_any_relaxed_accuracy']:.4f}`")
