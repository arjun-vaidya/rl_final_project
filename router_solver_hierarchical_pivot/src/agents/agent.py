...
ANSWER_BEARING_STEP_RE = re.compile(
    r"\b(answer|final|original question|overall|altogether|total|remaining|left|cost|sum|difference|product|quotient|how many|how much)\b",
    re.IGNORECASE,
)
...
class Rollout:
    ...
    retrieved_cases: List[Dict[str, object]] = field(default_factory=list)
...
class Agent:
    ...
    def _build_router_prompt(self, question: str, retrieved_cases: Optional[List[Dict[str, object]]] = None) -> str:
        retrieval_block = ""
        if retrieved_cases:
            rendered_cases = []
            for idx, case in enumerate(retrieved_cases[: self.hard_prompt_top_k], start=1):
                ...
                answer_bearing_text = ""
                if isinstance(answer_bearing_idx, int) and 0 <= answer_bearing_idx < len(subgoals):
                    answer_bearing_text = subgoals[answer_bearing_idx]
                if not answer_bearing_text and subgoals:
                    answer_bearing_text = str(subgoals[-1])
                key_subgoals = [str(s) for s in subgoals[:2]]
...
The final plan step must answer the original question directly, not compute an intermediate quantity.
...
JSON:"""
    ...
    def _build_solver_prompt(question: str, plan: List[str], previous_answers: List[str], step_idx: int, subgoal: str) -> str:
        ...
        if step_idx == len(plan) - 1:
            final_step_instruction = (
                "This is the final plan step. Use the previous steps to answer the original question directly.\n"
                "Do not return an intermediate value from an earlier step.\n\n"
            )
...
Do not end with prose after the final answer line."""
    ...
    @staticmethod
    def _build_easy_prompt(question: str) -> str:
        return f"""Solve this math problem directly.

Question: {question}

Show concise work if needed, then end with a final line exactly in the form:
Final answer: <number>
...
If the question asks how much remains or is still needed, prefer the remaining/difference quantity over subtotals."""
    ...
    @staticmethod
    def _is_answer_bearing_subgoal(text: str) -> bool:
        cleaned = " ".join(str(text).strip().split())
        if len(cleaned) < 12:
            return False
        return bool(ANSWER_BEARING_STEP_RE.search(cleaned))

    @staticmethod
    def _build_answer_bearing_final_step(question: str) -> str:
        return (
            "Answer the original question directly using the previous results. "
            f"State the final quantity requested in: {question}"
        )

    def _ensure_answer_bearing_final_step(self, question: str, plan: List[str]) -> List[str]:
        normalized = [" ".join(str(step).strip().split()) for step in plan if len(str(step).strip()) > 5]
        if not normalized:
            return []
        if self._is_answer_bearing_subgoal(normalized[-1]):
            return normalized[:8]

        final_step = self._build_answer_bearing_final_step(question)
        trimmed = normalized[:8]
        if len(trimmed) < 8:
            trimmed.append(final_step)
        else:
            trimmed[-1] = (
                f"{trimmed[-1]} Then answer the original question directly using the previous results."
            )
        return trimmed
    ...
    @staticmethod
    def _empty_token_tensor() -> torch.Tensor:
        return torch.empty(0, dtype=torch.long)

    def _rollout_easy_group(self, question: str, ground_truth: str, num_rollouts: int) -> List[Rollout]:
        self._set_adapter(self.solver_adapter)
        prompt = self._build_easy_prompt(question)
        generations = self._generate_same_prompt(
            prompt,
        ...
        plan = ["Answer the original question directly."]

        for solver_text, prompt_ids, comp_ids in generations:
            answer = self._extract_answer(solver_text)
            rollout = Rollout(
                question=question,
                ground_truth=ground_truth,
                router_prompt_ids=self._empty_token_tensor(),
                router_completion_ids=self._empty_token_tensor(),
                plan=plan.copy(),
                router_raw_text='{"branch":"easy"}',
                final_answer=answer,
                final_answer_source="easy_cot",
            )
    ...
    def rollout_group(self, question: str, ground_truth: str, num_rollouts: int) -> List[Rollout]:
        if self.execution_branch == "easy":
            return self._rollout_easy_group(question, ground_truth, num_rollouts)

        self._set_adapter(self.router_adapter)
        retrieved_cases = None
        if self.execution_branch == "hard" and self.graph_memory is not None:
            retrieved_cases = self.graph_memory.retrieve(question, k=self.retrieval_top_k)
        router_prompt = self._build_router_prompt(question, retrieved_cases=retrieved_cases)
        ...
        for router_text, router_prompt_ids, router_comp_ids in router_generations:
            ...
            if plan:
                plan = self._ensure_answer_bearing_final_step(question, plan)
            rollout = Rollout(
                question=question,
                ground_truth=ground_truth,
                ...
                retrieved_cases=[dict(case) for case in (retrieved_cases or [])],
            )
            if not plan:
