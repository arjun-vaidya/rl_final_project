...
OUT="experiments/router_objective_credit_o2_20260513_055220"
for VARIANT in baseline all_steps; do
  ...
  python3 -u main.py \
    ...
    --wandb-run-name "router_objective_o2_${VARIANT}_$(basename "$OUT")" \
  ...
  python3 -u main.py \
    ...
    --wandb-run-name "router_objective_o2_${VARIANT}_eval_$(basename "$OUT")" 2>&1 | tee "$OUT/o2_${VARIANT}_eval.log"
  python3 -u main.py \
    --mode taxonomy \
