...
OUT="experiments/router_objective_credit_o2_safe_20260513_060432"
for VARIANT in baseline all_steps; do
  ...
  python3 -u main.py \
    ...
    --wandb-run-name "router_objective_o2_safe_${VARIANT}_$(basename "$OUT")" \
  ...
  python3 -u main.py \
    ...
    --wandb-run-name "router_objective_o2_safe_${VARIANT}_eval_$(basename "$OUT")" 2>&1 | tee "$OUT/o2_${VARIANT}_eval.log"
  python3 -u main.py \
    --mode taxonomy \
