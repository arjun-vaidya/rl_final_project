...
def main():
    ...
    parser.add_argument("--execution-branch", choices=["easy", "soft", "hard"], default=None, help="Select which inference branch to execute")
    parser.add_argument("--memory-trace-input", default=None, help="Optional rollout trace JSONL used to build hard-branch retrieval memory")
    parser.add_argument("--memory-top-k", type=int, default=None, help="Number of retrieved memory entries for the hard branch")
    ...
    if args.execution_branch is not None:
        cfg.execution_branch = args.execution_branch
    if args.memory_trace_input is not None:
        cfg.memory_trace_input = args.memory_trace_input
    if args.memory_top_k is not None:
        cfg.memory_top_k = args.memory_top_k
    if args.memory_embedding_model is not None:
        cfg.memory_embedding_model = args.memory_embedding_model
    if args.memory_embedding_device is not None:
    ...
    print(
        ...
        f"Memory embedding: {cfg.memory_embedding_model} @ {cfg.memory_embedding_device}"
    )
    print()
